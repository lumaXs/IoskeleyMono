...
Processes only .ttf files directly in the input directory or one level
deep (width subfolders like Normal/, SemiCondensed/, Condensed/).
Skips TTF-Unhinted folders.
...
def fix_font(path, out_path):
    ...
    nerd_glyphs = {name for cp, name in cmap.items() if cp >= 0xE000}

    for gname in nerd_glyphs:
        try:
...
# Caminha pelo input_dir procurando TTFs
# Ignora qualquer pasta que contenha "Unhinted" no path
for root, dirs, files in os.walk(input_dir):
    # Pula pastas Unhinted
    dirs[:] = [d for d in dirs if 'Unhinted' not in d and 'unhinted' not in d]
